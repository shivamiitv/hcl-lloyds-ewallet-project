...
import com.hcl.lloyds.ewallet.service.PaymentGateway;
...
public class TransactionService {
    ...
    public void transfer(PaymentRequest req) {
        ...
        String pgRef;
        try {
            pgRef = paymentGateway.debit(req.getExternalCustomerId(), amt);
        } catch (Exception e) {
            throw new RuntimeException("Payment gateway debit failed", e);
        }

        boolean internalUpdated = false;
        ...
        try {
            ...
            credit.setReference(pgRef);
            transactionRepository.save(credit);

            internalUpdated = true;
        }
        finally {
            // COMPENSATION - if DB failed after PG debit
            if (!internalUpdated) {
                try {
                    paymentGateway.credit(req.getExternalCustomerId(), amt);
    ...
    public List<TransactionResponse> listForWallet(Long walletId) {
        ...
        return transactionRepository.findByWalletOrderByCreatedAtDesc(w)
                .stream()
                .map(t -> {
